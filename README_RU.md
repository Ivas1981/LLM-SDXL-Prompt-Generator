...
- 8-шаговый LLM-пайплайн (концепт → окружение → поза → состояние → освещение → камера → сборка → название) с опциональным пост-процессом через локальную модель
...
Все переменные перечислены в `env_registry.py` и могут быть проверены во время выполнения:
...
| `CONTEXT_TOKEN` | str | `{prompt}` | Плейсхолдер для описания внешности в позитивном промпте. Замените на своё описание перед использованием в SDXL. |
...
| `NSFW` | bool | `false` | При `true` шаг4_state включает дополнительное поле `nudity` в вывод |
...
`NSFW=true` в окружении, чтобы включить NSFW режим. Поведение:

- **SFW (по умолчанию):** используется `prompts/step4_state_sfw.txt` вместо
  `prompts/step4_state.txt`. SFW-промпт не содержит поля `nudity`.
- **NSFW:** используется `prompts/step4_state.txt` (уже включает поле
  `nudity`), а `prompts/step7_assemble_nsfw.txt` заменяет стандартный
  step7. Финальный позитив включает теги `nudity` после поля `camera`.
...
- **`{prompt}` сохраняется как есть** в выходном JSON. LLM инструктирован сохранять его как плейсхолдер. Пользователь заменяет его на своё описание объекта перед генерацией изображений.

## `{prompt}` плейсхолдер

Позитивный промпт всегда начинается с `{prompt}` (`CONTEXT_TOKEN`). Это плейсхолдер для описания внешности пользователя — женщина, возраст, цвет волос, телосложение, этническая принадлежность и т.д. LLM инструктирован сохранять `{prompt}` без изменений в выводе. Перед использованием в SDXL замените `{prompt}` на своё описание.

Пример: если сгенерированный промпт начинается с `{prompt}, clockwork engineer in worn leather apron...`, замените его на `молодая женщина с рыжими волосами, стройное телосложение, европеец, clockwork engineer in worn leather apron...`.
